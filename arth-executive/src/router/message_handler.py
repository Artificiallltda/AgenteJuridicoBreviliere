...
async def execute_brain(user_id: str, text: str, channel: str = "whatsapp", status_callback=None, user_name: str = "User", media_data: dict = None):
    ...
    try:
        ...
        # Se o grafo está parado no nó de aprovação, a última mensagem AI será o pedido de aprovação
        if final_state.next and "arth_approval" in final_state.next:
            return last_ai_msg.content if last_ai_msg else "[⚠️ Ação Crítica] Esta tarefa exige aprovação. Autoriza?"

        return last_ai_msg.content if last_ai_msg else "Processo finalizado."
